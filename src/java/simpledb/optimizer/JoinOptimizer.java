...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        //估计Cardinality，即估计产生结果的记录数
        int card;
        //Two types of joins: They are equality joins and range scans.
        if (joinOp == Predicate.Op.EQUALS) {
            if (t1pkey && t2pkey) { // Both are primary keys, choose the smaller one for estimation.
                card = Math.min(card1, card2);
            } else if (!t1pkey && !t2pkey) {
                // Both are not primary keys, choose the bigger one for estimation.
                card = Math.max(card1, card2);
            } else {
                // One of them is primary key, choose the non-primary key.
